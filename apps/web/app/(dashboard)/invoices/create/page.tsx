...
export default function CreateInvoicePage() {
    ...
    useEffect(() => {
        ...
        fetchQuotations().then(data => { if (data) setQuotations(data) })
    }, [])
    ...
    async function handleSubmit(formData: FormData) {
        ...
        const result = await addInvoice(formData, validItems)
    ...
    return (
        <div className="flex flex-col gap-6 p-8 max-w-4xl mx-auto">
            <div>
                ...
                <p className="text-muted-foreground">Bill a client for completed services or products.</p>
            </div>
            ...
            <form action={handleSubmit} className="space-y-6">
                <Card>
                    ...
                    <CardContent className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="grid gap-2">
                            <label htmlFor="linked_quotation_id" className="text-sm font-medium">Link Quotation (Optional)</label>
                            <select
                                name="linked_quotation_id"
                                id="linked_quotation_id"
                                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background"
                            >
                                <option value="">-- None --</option>
                                {quotations.map(q => (
                                    <option key={q.id} value={q.id}>{q.quotation_number} - ₹{q.grand_total}</option>
                                ))}
                            </select>
                        </div>
                    </CardContent>
                        ...
                        <div className="flex justify-end space-x-8">
                            <div className="space-y-4 w-64">
                                ...
                                <div className="pt-4 border-t flex justify-between items-center">
                                    ...
                                    <span className="text-lg font-bold text-red-600">Auto-calcualted</span>
                                </div>
                            </div>
                        </div>
